...
# Функции этого модуля реализуют полнотекстовый поиск среди постов и имен авторов или только среди имен авторов.
# Управляет типом поиска (посты и имена авторов или только имена авторов) параметр queryset_type.
...
# образовываться пуем добавления суффиксов и окончаний и отсекаем их, чтобы учесть еще больше возможных результатов.
    ...
    for element in src_queryset:
        ...
        for string in strings:
            ...
            if queryset_type == 'post':
                find_count += (string in element.text.lower()) or (string in element.account.username.lower()) or (
                        string in element.title.lower())
